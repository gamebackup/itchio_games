    ...
    eggsArr.forEach(e => {
        if (!e.collected && !e.isBeingDragged) {
            ...
            if (e.y >= UNDERGROUND_FLOOR_Y - 100 && e.y <= UNDERGROUND_FLOOR_Y) {
                ...
                if (state.hasStamper && e.type !== 'package' && !e.stamped && e.x >= 380 && e.x <= 420) {
                    e.stamped = true;
                    e.value = Math.floor(e.value * 2);
                }
...
function draw() {
    ...
    if (state.hasStamper) {
        // Left Leg only, in Background
        ctx.fillStyle = '#1a1a1a';
        ctx.fillRect(380, UNDERGROUND_FLOOR_Y - 60, 6, 65);  

        ctx.fillStyle = '#333';
        ctx.fillRect(380, UNDERGROUND_FLOOR_Y - 60, 40, 15); // Top housing
        
        let pressOffset = (Date.now() % 600 < 80) ? 35 : 0; // Much faster slam
        ctx.fillStyle = '#990000'; // slightly darker stamp press back
        ctx.fillRect(395, UNDERGROUND_FLOOR_Y - 45 + pressOffset, 10, 15); 
    }
    ...
    if (state.hasPackager) {
        ...
        // Packaging mechanical arm
        ctx.fillStyle = '#555';
        ctx.fillRect(528, UNDERGROUND_FLOOR_Y - 45, 4, 15 + Math.sin(Date.now()/200)*5); 
    ...
    if (state.hasRibbon) {
        ...
        ctx.fillRect(630, UNDERGROUND_FLOOR_Y - 50, 6, 55); // Left leg back
        ctx.fillRect(630, UNDERGROUND_FLOOR_Y - 50, 30, 8); // Top arch
    }
    ...
    if (state.hasStamper) {
        ...
        ctx.fillStyle = '#444';
        ctx.fillRect(414, UNDERGROUND_FLOOR_Y - 60, 6, 65);  
        ctx.fillStyle = '#222';
        ctx.fillRect(380, UNDERGROUND_FLOOR_Y - 50, 40, 4); // Front crossbar
    }

    if (state.hasRibbon) {
        // Front Right Leg (overlaps packages)
        ctx.fillStyle = '#ff3333';
        ctx.fillRect(654, UNDERGROUND_FLOOR_Y - 55, 6, 60); // Right front leg
        ctx.fillRect(630, UNDERGROUND_FLOOR_Y - 55, 30, 5); // Front top
    }
    ...
    particlesArr.forEach(p => {
        if (p.color) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText(p.text, p.x, p.y - (4 - p.life) * 10); // Float up
            ctx.restore();
        } else {
            ctx.fillStyle = `rgba(255, 255, 255, ${p.life})`;
            ctx.font = 'bold 12px sans-serif';
            ctx.fillText(p.text, p.x - 10, p.y - 20);
        }
    });
    ...
    ctx.font = '8px "Press Start 2P"';
    ctx.fillText(`v1.0.3 | ${currentFps} FPS`, canvas.width - 5, canvas.height - 5);
}
...
function buy(item) {
    if (state.money >= state.costs[item]) {
        ...
        if (item === 'chicken') {
            ...
            let gridSpacing = 16;
            let startX = (canvas.width - (35 * gridSpacing)) / 2;
            let startY = 173;
            let chickenIdx = 0;
...
function saveState() {
    ...
    localStorage.setItem('chickenIdleSave', JSON.stringify(saveData));
}
